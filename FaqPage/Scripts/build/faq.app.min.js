...
return angular.isArray(items)?items.forEach(function(item){for(var itemMatches=!1,keys=Object.keys(props),i=0;i<keys.length;i++){var prop=keys[i],text=props[prop].toLowerCase();if(-1!==item[prop].toString().toLowerCase().indexOf(text)){itemMatches=!0;break}}itemMatches&&out.push(item)}):out=items,out}}])}(),function(){"use strict";var animations={};animations.cshow=FAQRS.SimpleAnimation,animations.fadeIn=FAQRS.FadeIn,animations.fadeInUp=FAQRS.FadeInUp,animations.fadeInRight=FAQRS.FadeInRight,animations.fadeInLeft=FAQRS.FadeInLeft,animations.fadeInDown=FAQRS.FadeInDown,animations.bounceIn=FAQRS.BounceIn,animations.bounceInLeft=FAQRS.BounceInLeft,animations.bounceInRight=FAQRS.BounceInRight,animations.bounceInUp=FAQRS.BounceInUp,animations.bounceInDown=FAQRS.BounceInDown,animations.flip=FAQRS.Flip,animations.flipInX=FAQRS.FlipInX,animations.flipInY=FAQRS.FlipInY,animations.rotateIn=FAQRS.RotateIn,animations.rotateInDownLeft=FAQRS.RotateInDownLeft,animations.rotateInDownRight=FAQRS.RotateInDownRight,animations.rotateInUpLeft=FAQRS.RotateInUpLeft,animations.rotateInUpRight=FAQRS.RotateInUpRight,animations.zoomIn=FAQRS.ZoomIn,animations.zoomInUp=FAQRS.ZoomInUp,animations.zoomInRight=FAQRS.ZoomInRight,animations.zoomInLeft=FAQRS.ZoomInLeft,animations.zoomInDown=FAQRS.ZoomInDown,animations.pulse=FAQRS.Pulse,animations.swing=FAQRS.Swing,animations.tada=FAQRS.Tada,animations.lightSpeedIn=FAQRS.LightSpeedIn,animations.rollIn=FAQRS.RollIn,animations.wobble=FAQRS.Wobble,angular.module("FaqApp.services").value("animationList",animations)}(),function(){"use strict";angular.module("FaqApp.services").factory("context",[function(){function getParameterByName(name){name=name.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var regex=new RegExp("[\\?&]"+name+"=([^&#]*)"),results=regex.exec(location.search);return null===results?"":decodeURIComponent(results[1].replace(/\+/g," "))}function getWebUrl(key){var webUrl=getParameterByName(key);return webUrl.endsWith("/")?webUrl:webUrl+"/"}var editModeValue=getParameterByName("editmode"),editMode=!1;return editModeValue&&(editMode=!!parseInt(editModeValue,10)),{editMode:editMode,spHostUrl:getWebUrl("SPHostUrl"),spAppWebUrl:getWebUrl("SPAppWebUrl"),wpId:getParameterByName("wpId"),senderId:getParameterByName("SenderId")}}])}(),function(){"use strict";angular.module("FaqApp.services").factory("faqService",["context",function(context){return{appPartConfigRepository:new AppPartsConfigRepository(FAQ.Lists.AppPartsConfigUrl),faqRepository:new FaqRepository(FAQ.Lists.FaqUrl),webService:FAQ.SPWebService,resizeParent:function(){if(window.parent!==window){var $app=$jq("#s4-workspace"),message="<Message senderId="+context.senderId+" >resize("+$app.width()+","+($app.height()+40)+")</Message>";window.parent.postMessage(message,context.spHostUrl)}}}}])}(),function(){"use strict";angular.module("FaqApp.services").factory("faqSetModel",[function(){return{faqSetName:""}}])}(),function(){"use strict";angular.module("FaqApp.services").factory("$jq",function(){return window.jQuery})}(),function(){"use strict";angular.module("FaqApp.services").value("licenseStatus",{UnderTrial:1,Licensed:2,TrialExpired:3,LicenseNotValid:4}),angular.module("FaqApp.services").factory("licensing",["context","faqService","$q","licenseStatus",function(context,faqService,$q,licenseStatus){var licenseKey="_faq_license_",dateInstalledKey="_faq_date_installed",trialPeriodInDays=30,rsaKeyUrl="Content/rsa_public_key.txt",deferred=$q.defer(),checkTrialPeriod=function(dateInstalled){var dateNow=moment(),dayDIff=dateNow.diff(dateInstalled,"days");deferred.resolve(dayDIff>trialPeriodInDays?{status:licenseStatus.TrialExpired}:{status:licenseStatus.UnderTrial,daysLeft:trialPeriodInDays-dayDIff})};return{getLicenseStatus:function(){var licenseDefererd=faqService.webService.webProperties.get(licenseKey),hostWebIdDeferred=faqService.webService.getHostWebId();return $q.all([licenseDefererd,hostWebIdDeferred]).then(function(results){var license=results[0],hostWebId=results[1];
//if lecense provided
license?$jq.ajax({url:rsaKeyUrl,success:function(data){var decrypt=new JSEncrypt;decrypt.setPublicKey(data);var uncryptedHostWebId=decrypt.decrypt(license);deferred.resolve(hostWebId===uncryptedHostWebId?{status:licenseStatus.Licensed,licenseKey:license}:{status:licenseStatus.LicenseNotValid,licenseKey:license})},dataType:"text"}):faqService.webService.webProperties.get(dateInstalledKey).then(function(date){if(date)checkTrialPeriod(moment(date));else{var currentDate=new Date;faqService.webService.webProperties.set(dateInstalledKey,currentDate).then(function(){checkTrialPeriod(moment(currentDate))})}},function(error){deferred.reject(error)})},function(error){deferred.reject(error)}),deferred.promise},setLicense:function(license){return faqService.webService.webProperties.set(licenseKey,license)}}}])}(),function($){"use strict";angular.module("FaqApp.services").factory("processing",["$sce","$location","faqService",function($sce,$location,faqService){return{initilize:function($scope){$scope.Resources=FAQRS,$scope.trustedHtml=function(input){return $sce.trustAsHtml(input)},$scope.go=function(path){$location.path(path)},$scope.resizeParent=faqService.resizeParent,$scope.getInnerText=function(html){return $(String.format("<div>{0}</div>",html)).text().trim()}}}}])}(jQuery);
//# sourceMappingURL=faq.app.min.js.map